...
def preprocess_image(img_path: str, target_size: int) -> Tuple[np.ndarray, Tuple[int, int]]:
    ...
    target_size : int
        Target spatial size expected by the model.

    Returns
    -------
    Tuple[np.ndarray, Tuple[int, int]]
        Preprocessed image tensor (1, 3, H, W) and original image size (W, H).
    """
...
def rescale_centroids(centroids: List[Tuple[float, float]], input_size: int, original_w: int, original_h: int,) -> List[Tuple[float, float]]:
    """
    Rescale centroid coordinates to original image dimensions.
    """
...
def save_results(results: Dict, output_dir: str, image_name: str, image_path: str, visualize: bool = True) -> Dict[str, str]:
    """
    ...
    ----------
    results : Dict
        Inference results dictionary containing 'count', 'centroids', and 'inference_time_sec'.
    output_dir : str
        Directory path where results will be saved.
    image_name : str
        Base name of the image (without extension) for naming output files.
    image_path : str
        Path to the original input image (needed for visualization).
    visualize : bool, optional
        Whether to generate and save visualization plot. Default is True.
    
    Returns
    -------
    Dict[str, str]
        Dictionary with paths to saved files ('json_path' and optionally 'image_path').
    """
...
def run_inference(img_path: str, session: ort.InferenceSession, target_size: int) -> Dict:
    """
    ...
    -------
    Dict
        Inference results including count, centroids, and inference time.
    """
